    ...
    <div class="relative overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
        ...
        <flux:table>
            <flux:table.columns>
                ...
                <flux:table.column class="px-8 py-4">{{ __('Classification') }}</flux:table.column>
            ...
            <flux:table.rows>
                @forelse ($todos as $todo)
                    <flux:table.row>
                        <flux:table.cell class="py-5 pl-8">
                            <input
                                type="checkbox"
                                {{ $todo->completed ? 'checked' : '' }}
                                wire:change="toggle({{ $todo->id }})"
                                class="ml-2 size-4 rounded border-neutral-300 text-blue-600 focus:ring-blue-500"
                            />
                        </flux:table.cell>
                        ...
                        <flux:table.cell class="px-8 py-5">
                            @if ($todo->classification)
                                ...
                                @endphp
                                <flux:badge color="{{ $classificationColors[$todo->classification] ?? 'neutral' }}">
                                    {{ $classificationLabels[$todo->classification] ?? $todo->classification }}
                                </flux:badge>
                            @else
